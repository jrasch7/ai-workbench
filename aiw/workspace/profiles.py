"""Workspace profiles (load_workspaces_config, resolve_workspace, validate_profile etc) - aiw primary (surgical migration step).

Full heavy logic migrated here. aiw_workspace/profiles.py is now thin delegate reexporting from aiw/ (aiw-first).
Prefer: from aiw.workspace.profiles import ... or from aiw import resolve_workspace etc.

This contains the workspace profiles logic (not the agent profiles which live in aiw/profiles/loader.py).
No logic changes; pure move + reexports + minimal path adjustment for new location.
"""
...
AIW_ROOT = Path(__file__).resolve().parents[2]

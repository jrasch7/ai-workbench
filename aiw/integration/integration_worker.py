# MIGRAÇÃO CIRÚRGICA (Cockpit + agent): Worker de Integração em aiw/.
# aiw_workspace/integration_worker.py = thin delegate.
# Inclui policy checks. Foco em módulos para Loop/Cockpit.

"""Integration Worker (Provedor de Execução de Integração).

Executa ações de integração externa, como edição de PR no GitHub via gh cli.
Suporta dry-run (padrão) e execute com --confirm-external-send.
Registra attempts, aplica policy checks e marca itens como sent.

Alinha com o modelo de workers e "Loop Iterativo do Agente" para fluxos de patch.
"""
...
def _get_workspace_helpers():
    """Prefere aiw/ (resolve_workspace). AIW_ROOT calculado localmente. Migração integration completa para prefer aiw/.
    Termos PT: Worker de Integração, Outbox de Integração.
    """
...
def run_worker(workspace_id: str, item_id: str, dry_run: bool = True, confirm_external_send: bool = False, pr_number: str = None) -> dict:
    ...
    # aiw-first prefer (external_worker_policy migrated)
    try:
        from aiw.workspace.external_worker_policy import can_worker_execute
    except Exception:
        from aiw_workspace.external_worker_policy import can_worker_execute

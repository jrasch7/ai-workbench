# MIGRAÇÃO: thin delegate em aiw_workspace/integration_outbox.py aponta para aiw/.
# Outbox de Integração (para envios controlados). Suporta cockpit/agent flows sem tocar evidence full.

"""Integration Outbox para AIW.

Gerencia itens de saída (outbox) para integrações externas, como resumos de PR para GitHub.
Cria payloads baseados em evidence exports, com status draft/ready etc.
Usado junto com Integration Worker para envios controlados (dry-run por default).

Integra com evidências de revisão de patches.
"""
...
def _get_workspace_helpers():
    """Prefere aiw/ para resolve_workspace (reexport). AIW_ROOT via env sem tocar aiw_workspace.profiles.
    (evidence_export helpers ainda legacy por escopo cirúrgico; não tocamos módulos pesados.)
    """
...
def _get_evidence_export_helpers():
    from aiw_workspace.evidence_export import read_evidence_export, list_evidence_exports
    return read_evidence_export, list_evidence_exports
...
def create_outbox_item(workspace_id: str, patch_id: str, target: str, kind: str, export_id: str = None) -> dict:
    ...
    read_evidence_export, list_evidence_exports = _get_evidence_export_helpers()

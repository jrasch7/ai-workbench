"""Patch Gate (review/validation for patches) - aiw primary (surgical migration step 1/2).

Full logic migrated here. aiw_workspace/patch_gate.py is now thin delegate.
Prefer importing from aiw.patch.patch_gate or aiw.patch .
"""

import datetime
# aiw-first (step 1: test_runner + coverage_baseline now in aiw/patch/)
try:
    from .test_runner import load_patch_preview, list_test_runs
except Exception:
    from aiw_workspace.test_runner import load_patch_preview, list_test_runs
# aiw-first for migrated validation_plan + coverage_report (step 2)
from .validation_plan import (
    ensure_validation_plan_snapshot,
    compare_validation_plan_snapshots,
    validation_plan_for_patch
)
from aiw_workspace.test_coverage_intent import analyze_test_coverage_intent
...
def list_review_gates(workspace_id: str, status_filter: str = None, limit: int = 50) -> dict:
    ...
    # We need to iterate over patches (aiw-first step 1)
    try:
        from .test_runner import _workspace_base
    except Exception:
        from aiw_workspace.test_runner import _workspace_base

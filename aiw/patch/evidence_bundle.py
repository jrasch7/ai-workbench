# MIGRAÇÃO: Lógica movida de aiw_workspace/evidence_bundle.py para aiw/patch/evidence_bundle.py
# aiw_workspace/evidence_bundle.py agora thin delegate.
# Foco em alto valor: bundles de evidência para revisão/aprovação de patches.
# Depende de patch_gate, test_runner, validation_plan (ainda em legado durante migração segura).

"""Evidence Bundle para revisão de patches.

Agrega resultados de:
- Review Gate (patch_gate)
- Validation snapshots
- Cobertura (changed lines, baseline diff, report)
- Test results
- Risk summary computado

Suporta decisão (approved/rejected etc) e resumos para cockpit/scripts.
"""
...
def _get_patch_helpers():
    # aiw-first: prefer local migrated patch_gate + validation_plan
    from .patch_gate import review_gate_for_patch
    # aiw-first (step 1: test_runner)
    try:
        from .test_runner import get_test_run, load_patch_preview
    except Exception:
        from aiw_workspace.test_runner import get_test_run, load_patch_preview
    from .validation_plan import ensure_validation_plan_snapshot
    return review_gate_for_patch, load_patch_preview, ensure_validation_plan_snapshot, get_test_run
...
def create_evidence_bundle(workspace_id: str, patch_id: str) -> dict:
    ...
    review_gate_for_patch, load_patch_preview, ensure_validation_plan_snapshot, get_test_run = _get_patch_helpers()

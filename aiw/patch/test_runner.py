# MIGRAÇÃO CIRÚRGICA: Lógica principal movida de aiw_workspace/test_runner.py para aiw/patch/test_runner.py (test/cov related high-impact)
# aiw_workspace/test_runner.py agora thin delegate usando padrão lazy __getattr__ (como prev: validation/coverage/profiles).
# Prefer: from aiw.patch.test_runner import ... ; from aiw.patch import preview_test_command etc ; from aiw import ...
# Surgical: no behavior changes. Only move + import prefs (aiw-first) + path root fix (parents[2] for aiw/patch loc) + comments.
# Callers updated to prefer aiw/ or relative in patch pkg.
...
# aiw-first (profiles migrated)
try:
    from aiw.workspace.profiles import load_workspaces_config, resolve_workspace, validate_profile, validate_test_command
except Exception:
    from aiw_workspace.profiles import load_workspaces_config, resolve_workspace, validate_profile, validate_test_command
# aiw-first (coverage_report migrated to aiw/patch; thin delegate also available)
from aiw.patch.coverage_report import load_coverage_reports
...
def _workspace_base(workspace_id: str) -> Path:
    # adjusted for aiw/patch location (parents[2] reaches repo root, like other migrated patch modules)
    return Path(__file__).resolve().parents[2] / ".aiw" / "workspaces" / workspace_id
...
def run_test_command(
    ...
    from aiw_workspace.test_result_report import capture_test_result_report
...
def _patch_file(workspace_id: str, patch_id: str) -> Path:
    ...
    if scoped.is_file():
        return scoped
    legacy = Path(__file__).resolve().parents[2] / ".aiw" / "patches" / f"{safe_patch_id}.json"
    return legacy
